...
class DiscriminativeLoss(_Loss):
    ...
    def _cluster_means(self, input, target, n_clusters):
        ...
        for i in range(bs):
            ...
            # print(n_clusters[i])
            # n_clusters_int = n_clusters.round()
            # max_ind = n_clusters[i]
            # print(max_ind[0])
            # max_ind = max_ind[0].data.numpy().as_type(int)
            # print(max_ind)
            input_sample = input[i, :, :int(n_clusters[i])]
            # 1, n_clusters, n_loc,
            target_sample = target[i, :, :int(n_clusters[i])]
            # n_features, n_cluster
            mean_sample = input_sample.sum(2) / target_sample.sum(2)

            # padding
            n_pad_clusters = max_n_clusters - int(n_clusters[i])
    ...
    def _variance_term(self, input, target, c_means, n_clusters):
        ...
        for i in range(bs):
            # n_clusters, n_loc
            var_sample = var[i, :int(n_clusters[i])]
            # n_clusters, n_loc
            target_sample = target[i, :int(n_clusters[i])]
    ...
    def _distance_term(self, c_means, n_clusters):
        ...
        for i in range(bs):
            ...
            mean_sample = c_means[i, :, :int(n_clusters[i])]

            # n_features, n_clusters, n_clusters
            means_a = mean_sample.unsqueeze(2).expand(n_features, int(n_clusters[i]), int(n_clusters[i]))
            means_b = means_a.permute(0, 2, 1)
            diff = means_a - means_b

            margin = 2 * self.delta_dist * (1.0 - torch.eye(int(n_clusters[i])))
            margin = Variable(margin)
            if self.usegpu:
                margin = margin.cuda()
            c_dist = torch.sum(torch.clamp(margin - torch.norm(diff, self.norm, 0), min=0) ** 2)
            dist_term += c_dist / (2 * int(n_clusters[i]) * (int(n_clusters[i]) - 1))
        dist_term /= bs
